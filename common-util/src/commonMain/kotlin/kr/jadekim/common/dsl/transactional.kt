...
interface TransactionManager<O : TransactionManager.TransactionOptions> {
    ...
    companion object {
        ...
        suspend fun <T> inTransaction(name: String, options: (TransactionOptions.() -> Unit)? = null, block: suspend () -> T): T {
            val manager = managers[name] ?: throw IllegalArgumentException("No transaction manager found for key: $options")

            return manager.inTransaction(options, block)
        }
    }
...
suspend fun <T> transactional(name: String, options: (TransactionManager.TransactionOptions.() -> Unit)? = null, block: suspend () -> T): T =
