...
object Base64 : Encoder<ByteArray, String> {
    ...
    private val DECODE_MAP = IntArray(0x100).apply {
        ...
        for (n in CHARACTER_MAP.indices) {
            this[CHARACTER_MAP[n].code] = n
        }
    }

    private val DECODE_BYPASS_CHARS = listOf(' '.code.toByte(), '\n'.code.toByte(), '\r'.code.toByte())

    override fun encode(data: ByteArray): String = buildString((data.size * 4) / 3 + 4) {
        val extraBytes = data.size % 3
        var ipos = 0

        while (ipos < data.size - 2) {
            val num = data.readU24BE(ipos)
            ipos += 3
        ...
        if (extraBytes == 1) {
            ...
            val tmp = (data[ipos++].asU8() shl 8) or data[ipos++].asU8()
    ...
    override fun decode(data: String): ByteArray {
        val src = data.utf8().filter { !DECODE_BYPASS_CHARS.contains(it) }
