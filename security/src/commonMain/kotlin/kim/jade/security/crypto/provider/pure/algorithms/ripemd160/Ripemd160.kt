...
class Ripemd160 {
    ...
    internal object CryptographyAdapter : dev.whyoleg.cryptography.algorithms.Digest,
        dev.whyoleg.cryptography.operations.Hasher {

        override val id: dev.whyoleg.cryptography.CryptographyAlgorithmId<dev.whyoleg.cryptography.algorithms.Digest> =
            dev.whyoleg.cryptography.algorithms.RIPEMD160

        private val pool = ObjectPool { Ripemd160() }

        override fun hasher(): dev.whyoleg.cryptography.operations.Hasher = this

        override fun createHashFunction(): dev.whyoleg.cryptography.operations.HashFunction = Ripemd160HashFunction()

        class Ripemd160HashFunction : dev.whyoleg.cryptography.operations.HashFunction {

            private var digest: Ripemd160? = pool.acquire()
            ...
            override fun hashIntoByteArray(destination: ByteArray, destinationOffset: Int): Int {
                ...
                return digest.doFinal(destination, destinationOffset)
            }

            override fun hashToByteArray(): ByteArray {
                val digest = digest ?: throw IllegalStateException("Already closed")

                return ByteArray(DIGEST_LENGTH).apply { digest.doFinal(this, 0) }
            }
